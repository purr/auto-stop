...
# Read the real service version from config.py so the installer never goes stale.
$ServiceVersion = "1.0.0"
$configPyPath = Join-Path $ScriptDir "service\config.py"
if (Test-Path $configPyPath) {
    $vMatch = Select-String -Path $configPyPath -Pattern 'VERSION\s*=\s*"([^"]+)"' | Select-Object -First 1
    if ($vMatch) { $ServiceVersion = $vMatch.Matches[0].Groups[1].Value }
}
...
if ($existingVersion) {
    ...
    if ($existingVersion -eq $ServiceVersion -and -not $Force) {
        # Same version: still refresh the code + restart (fast), so re-running the installer
        # after editing service files always applies them. No -Force needed.
        Write-Info "Same version ($ServiceVersion) - refreshing service files and restarting."
    }
...
# Fast path: one Python launch that imports every dependency. If they all import, skip the
# slow per-package `pip show` checks entirely (the common case when re-running the installer).
$importProbe = 'import websockets, winrt.windows.media.control, winrt.windows.storage.streams, pystray, PIL, pycaw, psutil'
$probeOk = $false
try {
    if ($pythonCmd -eq "py -3") {
        & py -3 -c $importProbe 2>$null
    } else {
        & $pythonCmd -c $importProbe 2>$null
    }
    $probeOk = ($LASTEXITCODE -eq 0)
} catch { $probeOk = $false }

if ($probeOk) {
    Write-Success "All $totalPackages dependencies already installed"
} else {
    Write-Info "Checking/installing $totalPackages required packages..."
    Write-Host ""
    foreach ($pkg in $requiredPackages) {
        if (-not (Install-PythonPackage -PackageName $pkg.Name -MinVersion $pkg.MinVersion -Description $pkg.Description)) {
...
if (-not $NoStart) {
    ...
    try
    {
        ...
        # Poll for the service port instead of a fixed wait — succeeds the instant it's up.
        $portListening = $false
        for ($i = 0; $i -lt 20; $i++) {
            Start-Sleep -Milliseconds 200
            if (netstat -an | Select-String ":42089.*LISTENING") { $portListening = $true; break }
        }
        if ($portListening) {
        ...
        try
        {
            ...
            Start-Sleep -Seconds 1  # Wait for icon to appear
