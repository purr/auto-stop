...
class TelegramAudioController:
    """Detect and control Telegram Desktop audio via WASAPI.

    Telegram's video player does NOT register with the Windows System Media Transport
    Controls (SMTC), so it never appears in the media-session list the way WMP/Spotify do.
    We instead find its audio session with pycaw. That stack has no transport (play/pause)
    concept — the only lever is mute/unmute. So "pause" here means MUTE: the video keeps
    playing, we just silence it, which is what the one-media-at-a-time behaviour needs.
    """

    # Telegram Desktop and common forks.
    PROCESS_NAMES = {
        "telegram.exe",
    ...
    # AudioSessionState: 0 = Inactive, 1 = Active (feeding audio), 2 = Expired
    STATE_ACTIVE = 1
    ...
    def _find_session(self):
        ...
        try:
            for session in AudioUtilities.GetAllSessions():
                ...
                if proc and proc.name().lower() in self.PROCESS_NAMES:
                    return session
        except Exception as e:
            logger.debug(f"Telegram session lookup failed: {e}")
        return None

    def get_state(self) -> Optional[dict]:
        """Return {'active': bool, 'muted': bool} or None if Telegram has no audio session.

        `active` uses the session state (not the peak meter) so it stays True through silent
        scenes and while we have it muted — it only drops when Telegram stops feeding audio.
        """
        ...
        try:
            ...
            logger.debug(f"Telegram state read failed: {e}")
            return None

        try:
            active = int(session.State) == self.STATE_ACTIVE
        except Exception:
            # State unavailable: fall back to the peak meter. Fail CLOSED (not active) if that
            # also fails, so a COM read error can't spawn a phantom entry that pauses the browser.
            try:
                meter = session._ctl.QueryInterface(IAudioMeterInformation)
                active = meter.GetPeakValue() > 0.001
            except Exception:
                active = False

        return {"active": active, "muted": muted}

    def set_muted(self, muted: bool) -> bool:
        """Mute (pause) or unmute (resume) Telegram's audio session."""
        session = self._find_session()
        if not session:
            logger.warning("Telegram: no audio session found to mute/unmute")
            return False
        try:
            volume_ctl = session._ctl.QueryInterface(ISimpleAudioVolume)
            volume_ctl.SetMute(1 if muted else 0, None)
            logger.info(f"Telegram: {'muted' if muted else 'unmuted'}")
            return True
        except Exception as e:
            logger.error(f"Telegram set_muted({muted}) failed: {e}")
            return False


_telegram_controller: Optional["TelegramAudioController"] = None


def get_telegram_controller() -> "TelegramAudioController":
    """Get the Telegram controller singleton"""
    global _telegram_controller
    if _telegram_controller is None:
        _telegram_controller = TelegramAudioController()
