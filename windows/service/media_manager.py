...
class WindowsMediaManager:
    ...
    def __init__(self):
        ...
        # Telegram VIDEO detection (video-with-sound has no media session; detect-only).
        # We never control Telegram — "yield" just stops reporting it as active when the
        # browser takes over, so nothing oscillates. The video itself is never touched.
        self._telegram_video_active_since: Optional[datetime] = None
        self._telegram_video_yielded = False
    ...
    def _is_browser_app_id(self, app_id: str) -> bool:
        ...
        # Firefox on Windows often has a hex hash as app ID (e.g., "1A2B3C4D5E6F7890")
    ...
    async def start(self, on_state_change: Callable = None):
        ...
        try:
            ...
            # One-time cleanup: undo any Telegram mute left by an older version.
            self._reconcile_telegram_mute()
    ...
    async def _poll_media_state(self):
        ...
        while self._running:
            try:
                ...
                # Detect Telegram VIDEO so it can pause the browser (detection only; we
                # never mute or otherwise control Telegram).
                try:
                    await self._check_telegram_video()
                except Exception as e:
                    logger.error(f"Telegram video check error: {e}", exc_info=True)
    ...
    def _reconcile_telegram_mute(self):
        """One-time safety cleanup: if an older version of this service left Telegram's
        audio session muted, restore it. This service NEVER mutes a desktop app — Telegram
        and AyuGram are controlled through the normal Windows media-session path (real
        pause) when their build exposes it, exactly like Spotify/WMP.
        """
        if not PYCAW_AVAILABLE:
            return
        try:
            from audio_detector import get_telegram_controller

            controller = get_telegram_controller()
            state = controller.get_state()
            if state and state.get("muted"):
                logger.info("[Telegram] clearing a leftover mute from a previous version")
                controller.set_muted(False)
        except Exception as e:
            logger.debug(f"Telegram mute reconcile failed: {e}")

    async def _check_telegram_video(self):
        """Detect a Telegram/AyuGram VIDEO (video-with-sound) via its audio session so it can
        pause the browser while it plays. Telegram video exposes no external control, so this
        is DETECTION ONLY — the app's audio is never muted or touched. If the extension asks
        to 'pause' it (because browser media took over) we simply stop reporting it as active
        (yield); the video keeps playing. Songs are handled by the real media-session path.
        """
        ...
        session_id = "desktop-telegram-video"

        try:
            from audio_detector import get_telegram_controller

            state = get_telegram_controller().get_state()
        except Exception as e:
        ...
        if active:
            if self._telegram_video_active_since is None:
                self._telegram_video_active_since = now
        else:
            self._telegram_video_active_since = None
            self._telegram_video_yielded = False

        # Require the sound to persist a bit (~2 polls) so a single-poll blip / short
        # notification 'ding' isn't treated as a video, while staying responsive.
        is_video = active and (now - self._telegram_video_active_since) >= timedelta(seconds=0.4)

        if not is_video:
            if session_id in self._media_info:
                logger.info("[TelegramVideo] stopped - removing")
                del self._media_info[session_id]
                if self._on_state_change:
                    await self._on_state_change(self.get_state())
            return

        playing = not self._telegram_video_yielded
        old = self._media_info.get(session_id)
        was_playing = old.is_playing if old else False

        info = old or DesktopMediaInfo(
            session_id=session_id,
            app_id="Telegram (video)",
            title="Telegram video",
            duration=0,
            current_time=0,
        )
        info.is_playing = playing
        info.manually_paused = False
        info.last_update = now
        self._media_info[session_id] = info

        if playing and not was_playing:
            logger.info("[TelegramVideo] playing - pausing other desktop media")
            await self.pause_all_except(session_id)

        if old is None or was_playing != playing:
            logger.info(
                f"[TelegramVideo] {'playing (browser will pause)' if playing else 'yielded to browser (video keeps playing, not muted)'}"
            )
            if self._on_state_change:
                await self._on_state_change(self.get_state())
    ...
    async def _update_all_media_info(self):
        ...
        for stale_id in stale_ids:
            if stale_id.endswith("-fallback") or stale_id == "desktop-telegram-video":
                continue  # Synthetic entries are managed by their own checks
        ...
        for session_id, info in list(self._media_info.items()):
            if session_id.endswith("-fallback") or session_id == "desktop-telegram-video":
                continue  # Synthetic entries are managed separately
    ...
    async def play(self, session_id: str) -> bool:
        ...
        # Telegram video: "resume" just stops yielding. The video was never actually paused
        # (Telegram can't be controlled), so there's nothing to un-pause — just report it.
        if session_id == "desktop-telegram-video":
            logger.info("[TelegramVideo] resume request - un-yielding (video was never paused)")
            self._telegram_video_yielded = False
            return True
    ...
    async def pause(self, session_id: str) -> bool:
        ...
        # Telegram video: we can't actually pause it and we will NOT mute it. Just yield —
        # stop reporting it as active so the browser can take over without oscillating.
        if session_id == "desktop-telegram-video":
            logger.info("[TelegramVideo] pause request - yielding (NOT muting; video keeps playing)")
            self._telegram_video_yielded = True
            return True
    ...
    async def pause_all_except(self, except_session_id: Optional[str] = None) -> int:
        ...
        # Copy: self.pause() awaits, during which the poll task may add/remove entries
        # (e.g. the synthetic Telegram-video entry) and mutate this dict.
        for session_id, info in list(self._media_info.items()):
