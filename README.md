...
- **Works Everywhere**: YouTube, YouTube Music, SoundCloud, Bandcamp, Spotify Web, Twitch, TikTok, and any site with HTML5 audio/video — no per-site setup
...
- **Telegram / AyuGram**: **songs** get real play/pause (they show up in Windows media controls, like Spotify). **Videos** don't expose anything to Windows, so playing a Telegram video will pause your browser — but the reverse can't work (a Telegram video can't be paused from outside, and this never mutes your app). The service never mutes desktop apps.
